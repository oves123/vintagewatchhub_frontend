...
function ProfileContent() {
  ...
  const [shippingQuoteForm, setShippingQuoteForm] = useState({ dealId: null, amount: "" });
  ...
  const handleShippingQuote = async (dealId) => {
    if (!shippingQuoteForm.amount) return showToast("Enter a valid shipping quote", "error");
    setIsSubmittingQuote(true);
    try {
      const res = await fetch(`${API_URL}/orders/${dealId}/quote-shipping`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json", "Authorization": `Bearer ${localStorage.getItem("token")}` },
        body: JSON.stringify({ shipping_fee: shippingQuoteForm.amount })
      });
      const data = await res.json();
      if (res.ok) {
        showToast("Quote sent successfully!");
        setShippingQuoteForm({ dealId: null, amount: "" });
        loadActivity();
      } else {
        showToast(data.message || "Failed to send quote", "error");
      }
    } catch (err) {
      showToast("Error sending quote", "error");
    } finally {
                                      ...
                                      <span className={`text-xs font-black uppercase tracking-widest px-2 py-0.5 rounded-lg ${
                                        ...
                                        ['CANCELLED', 'REFUND_PENDING'].includes(deal.status) ? 'bg-rose-500 text-white' : 'bg-primary text-white'
                                      }`}>
                                         {
                                          ...
                                          deal.status === 'ACCEPTED' ? (deal.payment_status === 'AWAITING_QUOTE' ? 'WAITING FOR SHIPPING QUOTE' : 'ACTION REQUIRED: PAY NOW') : deal.status === 'REFUND_PENDING' ? 'REFUND PENDING' :
                                          deal.status
                                         }
                                      </span>
