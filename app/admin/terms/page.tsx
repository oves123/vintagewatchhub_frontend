      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-10 gap-6">
          <div>
            ...
            <p className="text-xs font-bold text-muted uppercase tracking-[0.2em] mt-1">Manage global marketplace terms and registration protocols</p>
          </div>
          <div className="flex gap-4">
            <Link href="/admin" className="px-6 py-3 bg-surface border border-border rounded-lg text-xs font-bold text-muted uppercase tracking-widest hover:text-foreground transition-all">
              Exit Settings
            </Link>
            <button 
              onClick={handleSave}
              disabled={saving}
              className="px-8 py-3 bg-primary text-white rounded-lg text-xs font-bold uppercase tracking-widest hover:bg-blue-700 transition-all shadow-lg shadow-blue-100 disabled:opacity-50"
            >
        ...
        {message && (
          <div className={`mb-8 p-4 rounded-xl text-sm font-black uppercase tracking-widest text-center ${message.includes('successfully') ? 'bg-emerald-50 text-emerald-600 border border-emerald-100' : 'bg-rose-50 text-rose-600 border border-rose-100'}`}>
            {message}
          </div>
        )}
            ...
            <div className="bg-surface p-8 rounded-3xl border border-border shadow-sm">
                ...
                <div className="space-y-3">
                   <div className="p-4 bg-blue-50/50 rounded-xl border border-blue-100">
                      <p className="text-xs font-black text-primary uppercase tracking-widest">Dynamic Variable</p>
            ...
            <div className="bg-foreground p-8 rounded-3xl shadow-xl">
                ...
                <div className="space-y-4">
                   <div className="flex justify-between items-center py-2 border-b border-white/10">
                      ...
                      <span className="text-xs text-emerald-400 font-bold uppercase tracking-widest underline decoration-2 underline-offset-4">Active</span>
                   </div>
                   <div className="flex justify-between items-center py-2">
                      <span className="text-xs text-white/40 font-bold uppercase tracking-widest">Last Update</span>
                      <span className="text-xs text-white font-bold uppercase tracking-widest">{new Date().toLocaleDateString()}</span>
                   </div>
                </div>
            </div>
          </div>
